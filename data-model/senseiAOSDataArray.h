...
public:
   ...
   */
  ValueType GetTypedComponent(svtkIdType tupleIdx, int comp) const SVTK_EXPECTS(0 <= tupleIdx &&
    tupleIdx < GetNumberOfTuples()) SVTK_EXPECTS(0 <= comp && comp < GetNumberOfComponents())
  {
   ...
   */
  void SetTypedComponent(svtkIdType tupleIdx, int comp, ValueType value) SVTK_EXPECTS(0 <= tupleIdx &&
    tupleIdx < GetNumberOfTuples()) SVTK_EXPECTS(0 <= comp && comp < GetNumberOfComponents())
  {
    const svtkIdType valueIdx = tupleIdx * this->NumberOfComponents + comp;
   ...
   * SVTK_DATA_ARRAY_FREE, free() will be used. If the delete method is
   * SVTK_DATA_ARRAY_DELETE, delete[] will be used. If the delete method is
   * SVTK_DATA_ARRAY_ALIGNED_FREE _aligned_free() will be used on Windows, while
   * free() will be used everywhere else. The default is FREE.
   */
  void SetArray(SVTK_ZEROCOPY ValueType* array, svtkIdType size, int save, int deleteMethod);
  void SetArray(SVTK_ZEROCOPY ValueType* array, svtkIdType size, int save);
   ...
   * @note This is a legacy method from svtkDataArrayTemplate, and is only
   * implemented for array-of-struct arrays. It currently just calls
   * DataChanged() and does nothing clever.
   * TODO this is only defined for AOS (svtkDataArrayTemplate leftover).
   * Deprecate to favor DataChanged?
   */
  void DataElementChanged(svtkIdType) { this->DataChanged(); }
  ...
  int GetArrayType() const override { return svtkAbstractArray::AoSDataArrayTemplate; }
